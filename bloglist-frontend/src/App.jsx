...
const handleLikeBlog = async (id, blogToUpdate) => {
  try {
    // 1. Prepare the payload: Ensure 'user' is only the ID string
    const blogToSend = {
      ...blogToUpdate,
    ...
    // 3. Update the state with the full user object restored
...
const handleRemoveBlog = async (blog) => {
  if (window.confirm(`Remove blog "${blog.title}" by ${blog.author}?`)) {
    try {
      ...
      // Update state: filter out the deleted blog
      setBlogs(blogs.filter(b => b.id !== blog.id))
      
      setNotificationType('success')
      setNotificationMessage(`Successfully removed "${blog.title}"`)
      
      // Navigate AFTER the state update to ensure the view changes
      navigate('/') 
    } catch (exception) {
    ...
    <Container maxWidth="md">
      <AppBar position="static" sx={{ mb: 4 }}>
        <Toolbar sx={{ gap: 2 }}>
          ...
          {user && <Link to="/create" style={{ color: 'white', textDecoration: 'none' }}>Create New</Link>}
          <Box sx={{ flexGrow: 1 }} />
          {user ? (
            <>
      ...
      <Typography variant="h4" gutterBottom>Blogs Application</Typography>
      ...
      <Routes>
        ...
        <Route path="/create" element={
          user ? <BlogForm createBlog={handleCreateBlog} /> : <Typography variant="h6">Please log in.</Typography>
        } />
        ...
        <Route path="/login" element={
          <Box component="form" onSubmit={handleLogin} sx={{ maxWidth: 400, mx: 'auto', mt: 5, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Typography variant="h5" align="center">Log in</Typography>
            <TextField label="Username" value={username} onChange={({ target }) => setUsername(target.value)} fullWidth />
            <TextField label="Password" type="password" value={password} onChange={({ target }) => setPassword(target.value)} fullWidth />
